...
const generateKeysHandler = async (c: any) => {
...
const getKeysInfoHandler = async (c: any) => {
  const apiKeys = c.env?.API_KEYS?.split(',').map((key: string) => key.trim()) || []

  return c.json({
    success: true,
    data: {
      totalKeys: apiKeys.length,
      keyPrefixes: apiKeys.map((key: string) => key.substring(0, 8) + '...'),
...
const adminHealthHandler = (c: any) => {
  return c.json({
    success: true,
    data: {

...
export const globalErrorHandler = (err, c) => {
    ...
    if (err instanceof ValidationError) {
        return errorResponse(c, 'VALIDATION_ERROR', err.message, { field: err.field }, 400);
    }
    if (err instanceof NotFoundError) {
        return errorResponse(c, 'NOT_FOUND', err.message, null, 404);
    }
    if (err instanceof ConflictError) {
        return errorResponse(c, 'CONFLICT', err.message, null, 409);
    }
    // Default error handling - no more process.env usage
    return errorResponse(c, 'INTERNAL_SERVER_ERROR', 'An unexpected error occurred', null, // Don't expose internal error details in production
    500);
};
